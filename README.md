...
3. Try enrolling again but the moment enrollment starts (the screen that says enrollment, not please wait) esc+refresh+power (it's timing sensitive so don’t expect to get it first try), and you are done, block_devmode is set to 0 and you can do BadRecovery unverified, which can help facilitate exploits/unenrollments like quicksilver on keyrolled devices (kv6). To enter an unverified recovery image, esc+refresh+power then ctrl+d and enter, esc+refresh+power again and plug in usb.
