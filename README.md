# sh1ttyexec
Exploit allowing for code execution on keyrolled chromebooks with kernel version 6, leading to unenrollment.
...
2. Start enrolling the device then open powerwash menu (ctrl+alt+shift+r) on the enrollment screen not the please wait and wait till it crashes back to OOBE
3. Try enrolling again but the moment enrollment starts (the screen that says enrollment, not please wait) esc+refresh+power (it's timing sensitive so don’t expect to get it first try), and you are done, block_devmode is set to 0 and you can do [badsh1mmer](https://github.com/crosbreaker/badsh1mmer), which can help facilitate exploits/unenrollments like quicksilver on keyrolled devices (kv6). To enter an unverified recovery image, esc+refresh+power then ctrl+d and enter, esc+refresh+power again and plug in a USB stick.
4. Get into [badsh1mmer](https://github.com/crosbreaker/badsh1mmer) using the steps shown above, then run the "quicksilver" payload to unenroll you're chromebook.
